...
int findLatestIndex() {
  for (int i=1; i<=TOTAL_INDEX; i++) {
    Serial.print("Check ");
    Serial.println(i);
    File file = SD.open(GetFileUri(i));
    if (!file) {
      return i;
    }
  }
...
//  Serial.println(file_pnt);
  ...
  if (file_pnt + len < FILE_MAX) {
    memcpy(filebuff+file_pnt, message, len);
    file_pnt += len;
  }
  else {
    ...
    file_index++;
    if (file_index > TOTAL_INDEX)
      file_index = 1;
    deleteFile(SD, GetFileUri(file_index));
  }
}
...
//  Serial.print("Found latest index is ");
...
//    WriteMessage("CURQA: 0.123456789\n");
//  }
